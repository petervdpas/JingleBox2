...
public sealed partial class WaveLevel : ObservableObject
{
    ...
    public string GainText => Math.Abs(Gain - 1) < SoundMachinePresetTools.SmallestMove
        ? "stays"
...
/// </remarks>
public sealed partial class SoundMachinePresetTools : ObservableObject
{
    ...
    public SoundMachinePresetTools(Func<SoundMachineProject?> open)
    {
        _open = open;
    ...
    private SoundMachinePresetSlot? preset;
    ...
    partial void OnPresetChanged(SoundMachinePresetSlot? value)
    {
        Wanted = value?.Name ?? "";
    ...
    public void Reread()
    {
        ...
        var found = new List<SoundMachinePresetSlot>();
        ...
        if (home.Length > 0 && Directory.Exists(home))
        {
            ...
            {
                found.Add(new SoundMachinePresetSlot(Called(path), path));
            }
        }
    ...
    /// </remarks>
    public ObservableCollection<PresetTool> Tools { get; } = new();
    ...
    public bool OnRename => Tool?.Key == PresetTool.Rename;

    /// <inheritdoc cref="OnList"/>
    public bool OnLevel => Tool?.Key == PresetTool.Level;
    ...
    /// </remarks>
    public IRelayCommand<PresetTool?> OpenCommand => new RelayCommand<PresetTool?>(one =>
    {
        if (one == null) return;
    ...
    private void Retool()
    {
        ...
        if (HasPreset)
            Tools.Add(new PresetTool(PresetTool.Rename, "Rename a preset",
                "The file, the name inside it, and the folder of recordings named after it."));

        if (HasMachine)
            Tools.Add(new PresetTool(PresetTool.Level, "Level recordings",
    ...
    private IReadOnlyList<string> Others(string folder, SoundMachinePresetSlot mine) =>
