...
/// </remarks>
public sealed partial class SoundMachinePresetDesk : ObservableObject
{
    ...
    public SoundMachinePresetDesk(Func<SoundMachineProject?> project) => _project = project;

    /// <summary>What the machine ships with, in the order its folder lists them.</summary>
    public ObservableCollection<SoundMachinePresetSlot> Presets { get; } = new();

    /// <summary>The one being edited. Setting it reads its file.</summary>
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(HasPreset))]
    private SoundMachinePresetSlot? picked;
    ...
    private SoundMachinePresetForm? form;
    ...
    /// </remarks>
    partial void OnPickedChanged(SoundMachinePresetSlot? value)
    {
        Problem = "";
        Said = "";
        Moved = false;
        ...
        try
        {
            ...
            if (read.ContainsKey(SoundMachinePresetWords.Browse))
            {
                Browses = true;
            ...
            if (!read.ContainsKey(SoundMachinePresetWords.Machine) && _project() is { } older)
            {
            ...
            Form = new SoundMachinePresetForm(
                read,
    ...
    public void Reread()
    {
        ...
        if (folder.Length > 0 && Directory.Exists(folder))
        {
            ...
            {
                Presets.Add(new SoundMachinePresetSlot(Named(path), path));
            }
        }
    ...
    public IRelayCommand SaveCommand => new RelayCommand(() =>
    {
        ...
        try
        {
            if (_project() is { } project) held[SoundMachinePresetWords.Machine] = project.Id;
    ...
    private int Sweep(SoundMachinePresetSlot one)
    {
        string home = Folder;
    ...
    private IReadOnlyList<string> Words(SoundDevices.SoundMachines.SoundMachineProject? machine) =>
        machine == null
            ? Array.Empty<string>()
            : new SoundMachineProjectShape(machine.Panel, machine.Parameters).ThingWords;

    /// <summary>True when that setting is the one holding a recording.</summary>
    private bool IsTake(string key) =>
        _project() is { } machine && new SoundMachineProjectShape(machine.Panel, machine.Parameters).IsTake(key);
