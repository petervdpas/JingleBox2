...
/// </remarks>
public sealed partial class PluginSlotViewModel : ObservableObject, Interfaces.IChainSlot
{
    ...
    public PluginSlotViewModel(PluginChainViewModel chain, IPluginEffect effect, PluginChain.Slot device)
    {
        _chain = chain;
        Effect = effect;
        Device = device;
    ...
    private System.Collections.Generic.List<ControlReading> Pick()
    {
        ...
        try
        {
            foreach (var parameter in Wanted())
            {
                ...
                found.Add(new ControlReading(reading.Name, reading.Text));
            }
        }
