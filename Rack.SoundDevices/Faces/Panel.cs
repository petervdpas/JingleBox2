...
/// <summary>
/// How a sound device's face is put together.
/// </summary>
/// <remarks>
/// A sound device is its parameters, but a list of parameters is not a panel. Which control turns
/// which parameter, what stands beside what, and what is grouped under one heading is a thing
/// the person who built the sound device knows and the host does not, so the sound device says it
/// here and the host draws what it is told.
///
...
/// to be extended, and every reader of it updated, before a sound device could use a new one.
///
/// What an element is called is a string rather than an enum for the same reason: a host that
/// meets an element it does not know draws nothing and carries on, which is what lets a sound
/// device built against a later library still open in an earlier host.
/// </remarks>
public sealed class PanelElement
{
    ...
    /// </summary>
    /// <remarks>
    /// A container has none. A control that names a parameter the sound device does not have is
    ...
    /// written. A reader takes the keys it understands and leaves the rest alone, so a sound device
...
/// </summary>
/// <remarks>
/// Written out as constants rather than an enum so a sound device can name one this host has never
...
{
    /// <summary>
    /// Rows and columns. Properties: columns, rows, and per child column, row, span.
    /// </summary>
    /// <remarks>
    ...
    /// name may take. They are worth saying where a sound device stands its knobs on a strip,
    /// because a row of dials that are not the same size does not line up with the row under it.
    ///
    ...
    /// position and reads out hertz, and only the sound device knows how one becomes the other.
    /// </remarks>
    ...
    /// above and its value below, so a sound device asking for a throw of 96 wants a control taller
    /// than that.
    ///
    /// Say nothing and the fader takes the standard throw, which is what nearly every sound device
    ...
    /// is a matter of what it does on this sound device, and a switch nobody has worded says on and
    ...
    /// be opened to find out what the sound device is set to.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Held, the parameter reads its top; let go, its bottom. A sound device that wants a thing to
    ...
    ///
    /// <c>pages</c> is false for the lamps alone, on a sound device with room for one and not the
    /// other. It names no parameter: where a song has got to is not a setting of the sound device,
    ...
    /// whoever is showing the panel, through <see
    /// cref="IPanelKeys"/>, the same way the pads do.
    /// On a kit they are the pads: the grid and the keyboard are two pictures of one thing, which
    /// is why picking a pad outlines its key.
    /// </remarks>
    ...
    ///
    /// The four are named rather than assumed, because a sound device is free to call its envelope
    /// whatever it likes and some sound devices have two.
    /// </remarks>
    public const string Envelope = "Envelope";

    /// <summary>
    /// The shape the sound device is making, drawn. Properties: width, height, cycles.
    /// </summary>
    /// <remarks>
    /// A row of knobs does not tell anybody what a wave with the duty at a fifth and the drive at
    /// four looks like, nor what a filter set that way is doing to a track. Either world can put
    /// one on its face.
    ///
    /// It turns nothing. The curve comes from the box's own engine through
    /// <see cref="IPanelScope"/>, because what a wave is and what drive does to it are the
    /// box's business, and a picture drawn from anything but the real thing would be a
    /// drawing of a sound device rather than of this one.
    ///
    /// <c>cycles</c> names the setting saying how much of the wave is shown. It is the one thing
    /// about this that anybody turns, and on the sound device it came from that knob is marked as
    /// no part of the sound: see <see cref="Parameter.Saved"/>.
    /// </remarks>
    public const string Scope = "Scope";

    /// <summary>
    /// A picture the sound device carries: a logo, a badge, a strip of trim.
    /// </summary>
    /// <remarks>
    /// Properties: file, width, height, and fit. It turns no parameter and never will. A badge
    /// is not a setting, nothing about it is worth writing into a song, and a sound device that
    /// wants its picture to change with what it is doing wants a control and not this.
    ///
    /// <c>file</c> names the picture inside the sound device's own folder, and names nothing else.
    /// A sound device travels as that folder, so a picture in it arrives with it, and a name is the
    ...
    /// came from whoever built the sound device, and where it may read from did not.
    ///
    ...
    /// <summary>
    /// Which recording the sound device plays. Property: caption.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Where the sound device is started from. Properties: source, caption, width.
    /// </summary>
    /// <remarks>
    /// The picker at the top of a panel. It turns no parameter, since what a preset does is write
    /// the lot of them at once, and it is not a Choice either: a Choice is a number the sound
    /// device keeps and this is not kept anywhere. The list comes from whoever is showing the
    /// panel, through <see cref="IPanelPresets"/>.
    ///
    /// There are two of these and they are not the same control. <c>source</c> says which:
    /// <see cref="PanelStarts.Presets"/> offers the sound device's own presets, which are a handful
    ...
    ///
    /// On the object, because that is where it is true. It used to be a fact about the sound
    /// device, which meant a sound device could not carry one of each and the answer had to be
    /// found by reading the presets folder to see whether anything in it said so. A sound device
    /// that says nothing here still falls back to that, so every sound device written before this
    ...
    /// says. A sound device that has just been pointed at a recording wants to write the name of it
    ...
    /// change it. What a pad on a kit is called is the player's word and not the sound device's, so
    ...
    /// belong to the preset or the song; whether it is lit is what the sound device is doing this
    /// instant. Those come from the host through <see
    /// cref="IPanelPads"/>, by the same position the
    /// buttons are declared in.
    ///
    /// A Pads holding no buttons falls back to however many the host has, which is what every
    /// sound device written before the buttons existed expects.
    /// </remarks>
    ...
    /// along one, and a preset written for a sound device is legible without counting.
    ///
    ...
    /// sound device's to say, unlike the pads of a kit: a zone is added and taken away while the
    /// instrument is being built, so the map comes from the host through
    /// <see cref="IPanelZones"/> and this draws what it is handed.
    /// </remarks>
    ...
    /// <summary>
    /// The recording the sound device holds, cut into pieces: the picture, the boundaries, and what
    /// it takes to make more or fewer of them.
    /// </summary>
    /// <remarks>
    /// A sound device that fills itself from one recording rather than many puts this on its face.
    /// It turns no parameter: where the cuts are is read back off the pieces, so the pieces are the
    /// truth and this is the picture of them. Supplied through <see
    /// cref="IPanelSlices"/> by whoever is showing
    /// the panel, for the same reason the pads are.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// A knob on a controller can be pointed at this sound device, and this is where the sound
    /// device says that belongs on its face: pressing it lists the desks a layout has been kept
    /// for, and one line more that starts learning, which is the same mode Ctrl+Shift+M turns over.
    ///
    /// It turns no parameter and never will. Which desk is plugged in and what has been kept for
    /// it are facts about the room the sound device is being played in, not about the sound device,
    /// so none of it can be written into a song and none of it is the sound device's to know. What
    /// is on offer comes from whoever is showing the panel, through <see cref="IPanelMenu"/>, the
    /// same way the presets and the map do.
    ///
    ...
    /// </summary>
    /// <remarks>
    /// The instrument's own name, which belongs to the song and not to the sound device: a sound
    /// device is called what the sound device is called, and an instrument off it is yours to call
    /// anything. So it turns no parameter and cannot: it comes from whoever is showing the panel,
    /// through
    /// <see cref="IInstrumentName"/>, the same way the presets and the map do, and it is read only
    /// where what is being shown is the sound device itself rather than an instrument.
    ///
    /// It is a part so that the sound device says where it goes. This program used to draw it in a
    /// corner over every panel, which is the one thing a sound device's face is never supposed to
    /// have done to it: a sound device that had never asked for it grew one, and a sound device
    /// with something of its own in that corner had the two drawn on top of each other.
    ///
    /// A sound device with no InstrumentName on it shows no name, which is a sound device saying
    /// its face is its own. Nothing is lost by that: the window is titled with it, the rack lists
    /// it, and the song's instrument list renames it.
    /// </remarks>
