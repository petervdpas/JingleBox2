...
/// the panel is standing: in a song it is the instrument's, in the sound device editor it is a
/// preview nobody keeps. Neither is any business of the drawing, so the panel is handed this
/// and asks it.
///
/// Values are doubles throughout, switches included, so that a sound device's settings are one kind
...
/// They are a second pair of methods and not a second interface, because it is one sound device's
/// settings either way and whoever holds them holds both.
/// </remarks>
public interface IPanelValues
{
    ...
    /// <remarks>
    /// Given a body so that everything holding a sound device's settings today still compiles: most
    /// sound devices are numbers from end to end and have no text to answer with. Nothing is the
    ...
    /// <summary>
    /// Sets it, because somebody picked something the sound device names rather than counts.
    /// </summary>
    ...
    ///
    /// Given a body so that anything holding a sound device's settings today still compiles. Not
