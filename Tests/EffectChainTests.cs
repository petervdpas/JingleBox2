...
public class EffectChainTests
{
    ...
    private static (PluginChain Chain, IEffectEngine Engine) Made(bool bypassed = false)
    {
        var chain = new PluginChain();
        var engine = new EffectEngines().Make(EffectEngines.EchoBox, Rate, Block)!;
    ...
    public void One_of_ours_is_written_down_as_its_id_and_its_knobs()
    {
        ...
        Assert.Equal(EffectEngines.EchoBox, saved.Effect);
    ...
    public void And_comes_back_the_same()
    {
        ...
        var device = Assert.Single(into.Devices);

        Assert.True(device.Bypassed);

        var engine = Assert.IsAssignableFrom<IEffectEngine>(device.Insert);

        Assert.Equal(EffectEngines.EchoBox, engine.Id);
    ...
    public void An_effect_this_build_has_not_got_is_named()
    {
        ...
        written.Devices.Add(new PluginDeviceConfig { Effect = "effect.somebody-elses", Name = "Whatever" });
    ...
    public void A_chain_from_before_effects_is_read_as_plugins()
    {
        ...
        written.Devices.Add(new PluginDeviceConfig { Id = "com.somebody.thing", Name = "Thing", Path = "/nowhere/thing.vst3" });
    ...
    public void The_face_writes_into_the_engine()
    {
        ...
        var values = new EffectValues(engine);
