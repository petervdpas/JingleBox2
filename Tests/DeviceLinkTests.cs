...
public class DeviceLinkTests
{
    ...
    public void A_machine_and_an_effect_are_offered_the_same_way()
    {
        ...
        Assert.Equal(ControlKind.Device, machine.Kind);
        Assert.Equal(ControlKind.Device, effect.Kind);
    ...
    public void An_effects_link_is_still_there_when_the_settings_are_read()
    {
        var kept = new List<ControlMapping>
        {
            ...
            new() { Kind = ControlKind.Insert, Plugin = "com.somebody.thing", Parameter = 12 }
        };
        ...
        Assert.DoesNotContain(kept, one => one.Kind == ControlKind.Insert);
    }
    ...
    public void Both_are_cut_into_cards_the_same_way()
    {
        ...
        Assert.Equal("device", Targets.KindOf(machine));
        Assert.Equal("device", Targets.KindOf(effect));
    ...
    /// <summary>
    /// A file says device, and one that says machine is read as what it always was.
    /// </summary>
    /// <remarks>
    /// Every template already on somebody's disc says machine, and every one of them is about a
    /// device. What is still refused is a plugin, which is what the word effect has always meant
    /// in one of these files.
    /// </remarks>
    [Fact]
    public void A_file_says_device_and_reads_machine_as_one()
    {
        Assert.NotNull(Targets.Point("device", "effect.echobox", "time"));
        Assert.NotNull(Targets.Point("machine", "machine.oddskilla", "cutoff"));

        Assert.Null(Targets.Point("effect", "com.somebody.thing", "12"));
        Assert.Null(Targets.Point("device", "effect.echobox", ""));
        Assert.Null(Targets.Point("device", "", "time"));
