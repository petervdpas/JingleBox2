...
A driver owns the card and can be handed one stream, so everything this application plays is
summed before it leaves: the tracker, the pads and a take being auditioned all go out of the
driver together. There is nothing to turn on for that and nothing to get wrong.
