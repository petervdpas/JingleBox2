...
/// through <see cref="IPanelScope"/>, so if the sound changes the picture changes with it, and
...
public class ScopeView : ScopeControl
{
    ...
    public static readonly StyledProperty<IPanelScope?> ScopeProperty =
        AvaloniaProperty.Register<ScopeView, IPanelScope?>(nameof(Scope));
    ...
    /// </remarks>
    private StreamGeometry Curve(IPanelScope scope, double width, double height)
    {
