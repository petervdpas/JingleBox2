...
/// <see cref="MachineProject"/>, an id is offered to <see cref="Machine.Register"/>,
...
public sealed class MachineRegistry : RackRegistry<MachineProject>
{
    ...
    {
        _archive = archive ?? new MachineArchive(this, new MachinePaths(Paths));
    }
    ...
    /// </remarks>
    protected override bool Register(MachineProject project) =>
        Machine.Register(project.Id, project.Name, project.Summary, project.Theme);

    /// <inheritdoc/>
    /// <remarks>
    /// A machine thrown out in SETTINGS has to be gone from the list the moment it is rebuilt,
    /// and the list of machines is a static one on <see cref="Machine"/> rather than
    /// whatever the caller does with what it is handed.
    /// </remarks>
    protected override void Forget() => Machine.Forget();

    /// <inheritdoc/>
    protected override bool Take(MachineProject project) => _archive.Add(project) != null;
}
